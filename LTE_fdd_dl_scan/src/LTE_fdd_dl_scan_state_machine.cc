...
#define MAX_PDSCH_ATTEMPTS    (50*10)
                      ...
                      gr::io_signature::make(MIN_OUT, MAX_OUT, sizeof(gr_complex)))
{
    ...
    // my own buffer
    my_i_buf          = (float *)malloc(SAMP_BUF_SIZE*sizeof(float));
    my_q_buf          = (float *)malloc(SAMP_BUF_SIZE*sizeof(float));
    my_samp_buf_w_idx = 0;
                                          ...
                                          gr_vector_void_star       &output_items)
{
    ...
    // define my own processing
    bool                        my_process_samples = false;
    static int 			cyclic_counter = 0;
    static uint32     		num_sumbframes = 0;
    ...
    if(freq_change_wait_done)
    {
        ...
        {
	    // initialize the i_buf and q_buf
	    //printf("sample buffer is updated with %u new samples starting from %u\n", ninput_items, samp_buf_w_idx);
            ...
            if(samp_buf_w_idx >= (SAMP_BUF_SIZE-(ninput_items+1)))
            {
                ...
                copy_input      = false; //we just copied
            }
        }else{
            // large write index will be reduced after processing
	    // we do not want it to be too empty, so copy
            process_samples = true;
            copy_input      = true;
        }
        
        // write my own buffer
        if(my_samp_buf_w_idx < (SAMP_BUF_SIZE-(ninput_items+1)))
        {
            my_copy_input_to_samp_buf(in, ninput_items);
            my_process_samples = false;
        }
        else
	{   // when it gets filled, use it, then reinitialize it
	    my_samp_buf_w_idx = 0;
            my_process_samples = true;
	}
    ...
    /*TCP-LTE*/
    // just check how fast it can be
    my_process_samples = false;
    /*TCP-LTE*/
    // do my own processing
    if(my_process_samples)
    {
      // next time the buffer gets full then we can process again
      my_process_samples = false;
      
      // dump the samples and check
      static int stored = 0;
      uint32 idx;
      if (stored < 0)
      {
	for(idx=0;idx< 153600*5+1;idx++)
	{
	  FILE *fp;
	  if(stored == 0)
	  {
	    fp = fopen("/tmp/signal_t0.txt", "a");
	  }
	  else
	  {
	    fp = fopen("/tmp/signal_t1.txt", "a");
	  }
	  fprintf(fp, "%f %f\n", my_i_buf[idx],my_q_buf[idx]);
	  fclose(fp);
	}
	stored++;
	fprintf(stderr, "write to file\n");
      ...
      // compute PRB utilization
      prb_utilization_ratio = liblte_phy_dl_spectrum_monitor(phy_struct, my_i_buf, my_q_buf, SAMP_BUF_SIZE-(ninput_items+1));
      printf("------ PRB utilization ratio for this buffer is %f\n", prb_utilization_ratio);
      
      
      
      
      // output the buffer level PRB utilization to the log file
      FILE *prb_buf_log;
      prb_buf_log = fopen("/tmp/prb_buf_log.txt", "a");
      fprintf(prb_buf_log, "%f\n", prb_utilization_ratio);
      fclose(prb_buf_log);
      
      prb_buf_log = fopen("/tmp/prb_utilization.txt", "w");
      fprintf(prb_buf_log, "%f\n", prb_utilization_ratio);
      fclose(prb_buf_log);
      
      // output to the file connecting to the MP4Client
//      int output_fd = open("/tmp/prb_utilization.txt", O_WRONLY | O_CREAT, 0644);
//      // convert the prb utilization ratio to a char array then send 
//      char prb_ratio_as_char[100];
//      sprintf( prb_ratio_as_char, "%f", prb_utilization_ratio );
//      write(output_fd, prb_ratio_as_char, sizeof(prb_ratio_as_char));
//      close(output_fd);
    }
    ...
    if(process_samples)
    {
              ...
              samp_buf_w_idx != 0)
        {
            ...
            case LTE_FDD_DL_SCAN_STATE_MACHINE_STATE_SSS_SEARCH:
                                                         ...
                                                         &frame_start_idx))
                {
                    ...
                    {
			//printf("****** SSS not found, go back to coarse time search ******\n");
                        // Go back to coarse timing search
                        state = LTE_FDD_DL_SCAN_STATE_MACHINE_STATE_COARSE_TIMING_SEARCH;
                        corr_peak_idx++;
                        init();
                    }else{
			//printf("****** SSS found, proceed to decode BCH ******\n");
                        // Decode BCH
			...
			//printf("PCI is %u, frame start index is %u\n",chan_data.N_id_cell, frame_start_idx);
			...
			//printf("****** BCH found, proceed to decode PDSCH for SIB1 ******\n");
			...
			// jump to my monitor
			state          = LTE_FDD_DL_SCAN_STATE_MACHINE_STATE_PRB_MONITOR;
			N_sfr          = 0;
			...
			//printf("****** Failed to find BCH, go back to coarse timing ******\n");
                        state          = LTE_FDD_DL_SCAN_STATE_MACHINE_STATE_COARSE_TIMING_SEARCH;
                        samp_buf_r_idx = 0;
                        samp_buf_w_idx = 0;
                        N_samps_needed = coarse_timing_search_num_samps;
            ...
            case LTE_FDD_DL_SCAN_STATE_MACHINE_STATE_PDSCH_DECODE_SIB1:
                if(LIBLTE_SUCCESS == liblte_phy_get_dl_subframe_and_ce(phy_struct,
                   ...
                   LIBLTE_SUCCESS == liblte_rrc_unpack_bcch_dlsch_msg(&rrc_msg,
                                                                      &bcch_dlsch_msg))
                {
		  fprintf(stderr,"PCI is %d, CFI is %d, ant num is %d\n", chan_data.N_id_cell, pcfich.cfi, N_ant);
                    // Send a PCAP message
		    ...
		    //printf("****** SIB1 in PDSCH decode success, proceed to decode all PDSCHs ******\n");
                    // Decode all PDSCHs
		    ...
		    //printf("************ no state change, try to deocde SIB1 again ******\n");
                ...
                {
		    //printf("************ max attempt to deocde SIB1 reached ******\n");
                    channel_found(switch_freq, done_flag);
                }
                break;
            case LTE_FDD_DL_SCAN_STATE_MACHINE_STATE_PDSCH_DECODE_SI_GENERIC:
                ...
                if(N_pdsch_attempts >= MAX_PDSCH_ATTEMPTS)
                {
		    //printf("************ Max attempt to decode PDSCH reached ******\n");
                    channel_found(switch_freq, done_flag);
                }else{
		  
		    //printf("************ Keep decoding other PDSCHs ******\n");
                    state          = LTE_FDD_DL_SCAN_STATE_MACHINE_STATE_PDSCH_DECODE_SI_GENERIC;
                    N_samps_needed = pdsch_decode_si_generic_num_samps;
                    N_sfr++;
                    if(N_sfr >= 10)
                ...
                break;
            case LTE_FDD_DL_SCAN_STATE_MACHINE_STATE_PRB_MONITOR:
	      
		   num_sumbframes = (num_sumbframes + 1)%65535; 
		   prb_utilization_ratio = -1; //negative inital value
		   
                //if(LIBLTE_SUCCESS == liblte_phy_get_dl_subframe_only(phy_struct,
                if(LIBLTE_SUCCESS == liblte_phy_get_dl_subframe_and_ce(phy_struct,
                                                                       i_buf,
                                                                       q_buf,
                                                                       samp_buf_r_idx,
                                                                     ...
                                                                     &pdcch))
                {
                   ...
                   prb_utilization_ratio = liblte_phy_prb_monitor(phy_struct, &subframe, chan_data.N_id_cell, N_ant);
		   fprintf(stderr,"subframe %d, prb utilization ratio is %f\n", num_sumbframes, prb_utilization_ratio);
		   
		   // output to sysctl interface
		   static float 	avg_prb_util 	= 0;
		   uint32 		bin_size 	= 100;
		   static uint32 	bin_counter 	= 0;
		   int 			prb_utilization_quantized;
		   
		   bin_counter++;
		   avg_prb_util += prb_utilization_ratio;
		   if(bin_counter == bin_size)
		   {
		     // average
		     avg_prb_util = avg_prb_util/bin_size; 
		     // write new value 
		      prb_utilization_quantized = (int)(63*avg_prb_util);
		      if (prb_utilization_quantized>63) {
			prb_utilization_quantized = 63;
		      }
		      
		      FILE *fp = fopen("/proc/sys/net/ipv4/tcp_prb", "w");
		      if(fp == NULL){
			fprintf(stderr, "this kernel does not support tcp_prb!\n");
		      }
		      fprintf(fp, "%d", prb_utilization_quantized);
		      fclose(fp);
		    ...
		    // keep monitoring the channel;
                    state          = LTE_FDD_DL_SCAN_STATE_MACHINE_STATE_PRB_MONITOR;
                    //N_samps_needed = pdsch_decode_si_generic_num_samps;
                    N_samps_needed = pdsch_decode_si_generic_num_samps;
		    
		    // keep counting the subframe number
		    // if we get 10 subframes, then move the read index forward by one frame
		      // ensure we have enough number of samples to process
		    if (((num_sumbframes % 500) ==499) && (samp_buf_w_idx - samp_buf_r_idx > 153600))
		    {
		      fprintf(stderr, "periodically start resync\n");
		      fprintf(stderr,"old index %d\n", samp_buf_r_idx); 
		      liblte_phy_get_dl_resync(phy_struct, i_buf, q_buf, &samp_buf_r_idx, chan_data.N_id_cell, N_ant);
		      fprintf(stderr,"new index %d, wirte index %d\n", samp_buf_r_idx, samp_buf_w_idx); 
		      //set the subframe back to 0 after resync
		      N_sfr = 0;
		    }
		    else
		    {
		      N_sfr++; //move the counter forward only when the sync works
		    }
        ...
        }// end on the while loop
        
        
        // Copy remaining samples to beginning of buffer
        // so we do not lose synchronization
        if(samp_buf_r_idx > 100)
        {
	    //cyclic_counter++; 
	    //fprintf(stderr,"=====we cyclic the inupt now with counter %d\n", cyclic_counter);
	...
	//    }
	    ...
	    //printf("+++++ write index is %u, read index is %u\n", samp_buf_w_idx, samp_buf_r_idx);
            samp_buf_w_idx  = 0;
            freq_shift(samp_buf_r_idx, samps_to_copy, -timing_struct.freq_offset[corr_peak_idx]);
	    // the write index just keeps increasing
        ...
        // this is never executed in experiments
        if(true == copy_input)
        {
            copy_input_to_samp_buf(in, ninput_items);
        }
...
void LTE_fdd_dl_scan_state_machine::my_copy_input_to_samp_buf(const gr_complex *in, int32 ninput_items)
{
    uint32 i;

    for(i=0; i<ninput_items; i++)
    {
        my_i_buf[my_samp_buf_w_idx]   = in[i].real();
        my_q_buf[my_samp_buf_w_idx++] = in[i].imag();
    }
}
