...
int main(int argc, char *argv[])
{
    ...
    printf("****** Estimate the PRB utilization ******\n");
    printf("Please connect to control port %u\n", LTE_FDD_DL_SCAN_DEFAULT_CTRL_PORT);
    
    // remove the log file which you will append during the process
    remove("/tmp/reference.txt");// our log file
    remove("/tmp/prb_buf_log.txt");// our log file
    remove("/tmp/signal_t.txt");// our log file
    
    remove("/tmp/signal_t0.txt");// our log file
    remove("/tmp/signal_t1.txt");// our log file
    remove("/tmp/symbol_lte.txt");// our log file
